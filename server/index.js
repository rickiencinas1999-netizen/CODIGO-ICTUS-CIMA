...
const upsertCaso = db.prepare(`
  INSERT INTO casos (id, folio, fecha, estado, duracion_min, capturado_por, informe, data, updated_at)
  VALUES (@id, @folio, @fecha, @estado, @duracion_min, @capturado_por, @informe, @data, @updated_at)
...
app.post('/api/casos', (req, res) => {
  ...
  const { id, folio, fecha, estado, duracionMin, capturadoPor, informe, ...extra } = record;
  upsertCaso.run({
    id,
    folio: folio ?? null,
    fecha: fecha ?? null,
    estado: estado ?? null,
    duracion_min: typeof duracionMin === 'number' ? duracionMin : null,
...
app.listen(PORT, () => {
  console.log(`Código Oro escuchando en http://localhost:${PORT}`);
});
